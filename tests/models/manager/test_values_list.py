...
async def test_model_values():
    ...
    users = await User.objects.values_list()
...
async def test_model_values_only_with_only():
    ...
    users = await User.objects.only("name", "language").values_list()
    assert len(users) == 2
...
async def test_model_values_list_fields():
    ...
    users = await User.objects.values_list(["name"])
...
async def test_model_values_list_flatten():
    ...
    users = await User.objects.values_list(["name"], flat=True)
...
async def test_raise_exception(value):
    with pytest.raises(FieldDefinitionError):
        await User.objects.values_list(value)
...
async def test_raise_exception_on_flatten_non_field():
    ...
    with pytest.raises(FieldDefinitionError):
        await User.objects.values_list("age", flat=True)
...
async def test_model_values_exclude_fields():
    ...
    users = await User.objects.values_list(exclude=["name", "id"])
    assert len(users) == 2
...
async def test_model_values_exclude_and_include_fields():
    user1 = await User.objects.create(name="John", language="PT")
    user2 = await User.objects.create(
        name="Jane", language="EN", description="Another simple description"
    )

    users = await User.objects.values_list(["id"], exclude=["name"])
    assert len(users) == 2
...
async def test_model_values_exclude_none():
    ...
    users = await User.objects.values_list(exclude_none=True)
    assert len(users) == 2
...
async def test_model_only_with_filter():
    ...
    users = await User.objects.filter(id=user2.id).values_list("name")
    assert len(users) == 1

    assert users == [("Jane",)]

    users = await User.objects.filter(id=3).values_list("name")
