...
async def test_model_bulk_update_many() -> None:
    ...
    with pytest.raises(InvalidKeyError):
        await Movie.objects.filter(name="Boyhood 2").update_many(test=2021)
...
async def test_model_bulk_update() -> None:
    ...
    with pytest.raises(InvalidKeyError):
        await Movie.objects.filter(name="Boyhood 2").update(test=2021)
...
async def test_model_bulk_update_function() -> None:
    ...
    with pytest.raises(InvalidKeyError):
        await Movie.objects.filter(name="Boyhood 2").bulk_update(test=2021)
