...
* **autogenerate_index** - Marks indexes intended for application-startup generation. Run
`await registry.document_checks()` from the Registry's application event loop.
...
When those are declared with `autogenerate_index=True`, run `await registry.document_checks()`
during async application startup. Index I/O is not performed while the document class is imported,
because the Registry client belongs to the application event loop.

You can also [manually create](#index-operations) indexes for an individual document.
...
When indexes are declared with `autogenerate_index=True`, call `await registry.document_checks()`
from application startup to create and reconcile them on the Registry's event loop.
...
!!! Tip
    Use `create_index()` when you need to create one index independently of application startup.
