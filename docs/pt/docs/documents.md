...
Quando forem declarados com `autogenerate_index=True`, execute `await registry.document_checks()`
durante o arranque assíncrono da aplicação. A criação dos índices não ocorre durante a importação da
classe, pois o cliente do registo pertence ao ciclo de eventos da aplicação.
...
!!! Tip
    Use `create_index()` quando precisar criar um único índice fora do arranque da aplicação.
